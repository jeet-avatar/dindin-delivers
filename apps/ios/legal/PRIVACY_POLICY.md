# Privacy Policy

**Dollor AI Service**

**Last Updated: December 7, 2024**

**Effective Date: December 7, 2024**

---

## Introduction

Dollor AI Service ("Dollor," "we," "us," or "our") is committed to protecting your privacy. This Privacy Policy explains how we collect, use, disclose, and safeguard your information when you use our mobile applications (Dollor Customer App, Dollor Driver App, and Dollor Restaurant App) and related services (collectively, the "Services").

Please read this Privacy Policy carefully. By using our Services, you consent to the practices described in this policy.

---

## Table of Contents

1. [Information We Collect](#1-information-we-collect)
2. [How We Use Your Information](#2-how-we-use-your-information)
3. [How We Share Your Information](#3-how-we-share-your-information)
4. [Data Retention](#4-data-retention)
5. [Data Security](#5-data-security)
6. [Your Privacy Rights](#6-your-privacy-rights)
7. [Children's Privacy](#7-childrens-privacy)
8. [International Data Transfers](#8-international-data-transfers)
9. [Third-Party Services](#9-third-party-services)
10. [Updates to This Policy](#10-updates-to-this-policy)
11. [Contact Us](#11-contact-us)
12. [Additional Disclosures](#12-additional-disclosures)

---

## 1. Information We Collect

### 1.1 Information You Provide Directly

**Account Information:**
- Full name
- Email address
- Phone number
- Password (encrypted)
- Profile photo (optional)

**Payment Information:**
- Credit/debit card details (processed by Stripe)
- Billing address
- Transaction history

**Delivery Information:**
- Delivery addresses
- Delivery instructions
- Saved locations (home, work, etc.)

**Driver Information (Drivers Only):**
- Driver's license details
- Vehicle information (make, model, year, license plate)
- Insurance documentation
- Bank account information for payments
- Background check consent

**Restaurant Information (Restaurants Only):**
- Business name and address
- Business license information
- Menu items and pricing
- Operating hours
- Bank account information for payments

**Communication Data:**
- Customer support inquiries
- In-app messages
- Feedback and reviews
- Survey responses

### 1.2 Information Collected Automatically

**Device Information:**
- Device type and model
- Operating system and version
- Unique device identifiers
- Mobile network information
- IP address

**Location Information:**
- Precise GPS location (with permission)
- Approximate location based on IP
- Location history during active orders/rides

**Usage Information:**
- App features used
- Pages and screens viewed
- Search queries
- Order history
- Interaction timestamps
- App performance data

**Log Data:**
- Access times and dates
- App crashes and errors
- System activity
- Referral URLs

### 1.3 Information from Third Parties

**Social Login Providers:**
- Google: Name, email, profile photo
- Apple: Name, email (may be anonymized per user choice)

**Payment Processors:**
- Transaction status
- Payment verification results

**Background Check Providers (Drivers):**
- Criminal history results
- Driving record information

**Map Services:**
- Route information
- Traffic data
- Address verification

---

## 2. How We Use Your Information

### 2.1 To Provide and Improve Services

- Process and fulfill orders and ride requests
- Connect customers with restaurants and drivers
- Calculate fares and delivery fees
- Process payments and refunds
- Provide customer support
- Send order updates and notifications
- Improve app functionality and user experience
- Develop new features and services

### 2.2 For Safety and Security

- Verify user identity
- Conduct driver background checks
- Detect and prevent fraud
- Investigate suspicious activity
- Ensure platform safety
- Comply with legal obligations

### 2.3 For Communication

- Send transactional notifications (order updates, receipts)
- Provide customer support responses
- Send promotional messages (with consent)
- Deliver important service announcements
- Request feedback and reviews

### 2.4 For Analytics and Research

- Analyze usage patterns and trends
- Measure service performance
- Conduct market research
- Generate aggregated insights
- Improve matching algorithms

### 2.5 For Legal and Compliance

- Comply with applicable laws and regulations
- Respond to legal requests and court orders
- Enforce our Terms of Service
- Protect our legal rights and interests

---

## 3. How We Share Your Information

### 3.1 With Other Users

**Customers see:**
- Driver's first name and photo
- Vehicle information
- Real-time location during delivery/ride
- Driver ratings

**Drivers see:**
- Customer's first name
- Pickup and delivery addresses
- Order details
- Customer ratings

**Restaurants see:**
- Customer's first name
- Delivery address
- Order details
- Special instructions

### 3.2 With Service Providers

We share information with third parties who perform services on our behalf:

| Provider | Purpose | Data Shared |
|----------|---------|-------------|
| Stripe | Payment processing | Payment card details, transaction data |
| Firebase | Authentication, notifications | Email, device tokens |
| Google Maps | Navigation, location services | Location data |
| AWS | Cloud hosting | All service data (encrypted) |
| Background check providers | Driver verification | Driver personal information |

### 3.3 For Legal Reasons

We may disclose information:
- To comply with legal obligations
- In response to lawful requests by public authorities
- To protect our rights, privacy, safety, or property
- In connection with legal proceedings
- To investigate potential Terms violations

### 3.4 Business Transfers

In the event of a merger, acquisition, or sale of assets, your information may be transferred to the acquiring entity.

### 3.5 With Your Consent

We may share information for other purposes with your explicit consent.

### 3.6 Aggregated or De-identified Data

We may share aggregated or de-identified data that cannot reasonably be used to identify you.

---

## 4. Data Retention

### 4.1 Retention Periods

| Data Type | Retention Period |
|-----------|------------------|
| Account information | Duration of account + 3 years |
| Transaction records | 7 years (tax/legal requirements) |
| Location data | 90 days after trip completion |
| Support communications | 3 years |
| Marketing preferences | Until consent withdrawn |
| Device information | 2 years from last app use |

### 4.2 Deletion

When data is no longer needed:
- Personal data is deleted or anonymized
- Backups are purged within 90 days
- Some data may be retained for legal compliance

---

## 5. Data Security

### 5.1 Security Measures

We implement industry-standard security measures:

- **Encryption:** All data encrypted in transit (TLS 1.3) and at rest (AES-256)
- **Access Controls:** Role-based access, multi-factor authentication
- **Infrastructure:** Secure cloud hosting with AWS
- **Monitoring:** 24/7 security monitoring and intrusion detection
- **Audits:** Regular security assessments and penetration testing
- **Training:** Employee security awareness training

### 5.2 Password Security

- Passwords are hashed using bcrypt
- We never store plaintext passwords
- Password reset requires email verification

### 5.3 Payment Security

- PCI DSS compliant payment processing
- Card details handled exclusively by Stripe
- We never store full card numbers

### 5.4 Breach Notification

In the event of a data breach affecting your personal information:
- We will notify you within 72 hours
- We will notify relevant authorities as required by law
- We will provide guidance on protective steps

---

## 6. Your Privacy Rights

### 6.1 Access and Portability

You have the right to:
- Access your personal data
- Request a copy of your data in a portable format
- Review information in your account settings

### 6.2 Correction

You can update or correct your information:
- Through app settings
- By contacting support@dollor.ai

### 6.3 Deletion

You can request deletion of your account and personal data:
- Through app settings (Delete Account option)
- By emailing privacy@dollor.ai
- Note: Some data may be retained for legal compliance

### 6.4 Opt-Out Rights

You can opt out of:
- **Marketing emails:** Unsubscribe link in emails or app settings
- **Push notifications:** Device settings or app settings
- **Location tracking:** Device settings (may limit service functionality)
- **Targeted advertising:** Device settings (Limit Ad Tracking)

### 6.5 Do Not Track

Our Services do not currently respond to "Do Not Track" browser signals.

### 6.6 Exercising Your Rights

To exercise any privacy rights:
- Email: privacy@dollor.ai
- In-app: Settings > Privacy > Manage Data
- Response time: Within 30 days

---

## 7. Children's Privacy

### 7.1 Age Requirement

Our Services are not intended for children under 18 years of age. We do not knowingly collect personal information from children under 18.

### 7.2 Parental Notice

If we learn that we have collected personal information from a child under 18, we will:
- Delete the information promptly
- Notify the parent or guardian if contact information is available

### 7.3 Reporting

If you believe we have collected information from a child, please contact us immediately at privacy@dollor.ai.

---

## 8. International Data Transfers

### 8.1 Data Location

Your information may be transferred to and processed in:
- United States
- Other countries where our service providers operate

### 8.2 Transfer Safeguards

For international transfers, we use:
- Standard Contractual Clauses approved by relevant authorities
- Data processing agreements with service providers
- Encryption for data in transit

### 8.3 Your Consent

By using our Services, you consent to the transfer of your information to the United States and other jurisdictions.

---

## 9. Third-Party Services

### 9.1 Third-Party Links

Our Services may contain links to third-party websites or services. We are not responsible for their privacy practices.

### 9.2 Third-Party Integrations

We integrate with:

**Google Services:**
- Google Sign-In (authentication)
- Google Maps (navigation, location)
- Firebase (notifications, analytics)
- Privacy Policy: https://policies.google.com/privacy

**Apple Services:**
- Apple Sign-In (authentication)
- Apple Maps (navigation)
- Apple Push Notifications
- Privacy Policy: https://www.apple.com/privacy

**Stripe:**
- Payment processing
- Privacy Policy: https://stripe.com/privacy

### 9.3 Analytics

We use analytics to improve our Services:
- Firebase Analytics
- Crash reporting tools

You can opt out of analytics through device settings.

---

## 10. Updates to This Policy

### 10.1 Notification of Changes

We may update this Privacy Policy periodically. We will notify you of material changes through:
- In-app notifications
- Email to your registered address
- Updated "Last Modified" date

### 10.2 Review

We encourage you to review this Privacy Policy regularly.

### 10.3 Continued Use

Your continued use of our Services after changes constitutes acceptance of the updated policy.

---

## 11. Contact Us

### 11.1 Privacy Questions

For privacy-related questions or concerns:

**Email:** privacy@dollor.ai

**Support:** support@dollor.ai

**Website:** https://dollor.ai/privacy

### 11.2 Data Protection Officer

For formal privacy requests:

**Email:** dpo@dollor.ai

### 11.3 Mailing Address

Dollor AI Service
Privacy Team
[Your Business Address]

### 11.4 Response Time

We aim to respond to all privacy inquiries within 30 days.

---

## 12. Additional Disclosures

### 12.1 California Privacy Rights (CCPA)

If you are a California resident, you have additional rights under the California Consumer Privacy Act:

**Right to Know:**
- Categories of personal information collected
- Sources of personal information
- Business purposes for collection
- Categories of third parties with whom we share

**Right to Delete:**
- Request deletion of personal information
- Subject to legal exceptions

**Right to Opt-Out:**
- Opt out of sale of personal information
- Note: We do not sell personal information

**Right to Non-Discrimination:**
- We will not discriminate against you for exercising your rights

**Shine the Light:**
- California residents may request information about disclosure of personal information to third parties for direct marketing purposes

To exercise California rights, contact: privacy@dollor.ai

### 12.2 Nevada Privacy Rights

Nevada residents may opt out of the sale of personal information. We do not currently sell personal information as defined under Nevada law.

### 12.3 European Privacy Rights (GDPR)

If you are in the European Economic Area, you have additional rights:

**Legal Basis for Processing:**
- Contract performance (providing Services)
- Legitimate interests (improving Services, security)
- Legal compliance
- Consent (marketing communications)

**Additional Rights:**
- Right to restriction of processing
- Right to object to processing
- Right to data portability
- Right to withdraw consent
- Right to lodge a complaint with a supervisory authority

**Data Controller:**
Dollor AI Service is the data controller for your personal information.

### 12.4 Location Data Disclosure

**When Location is Collected:**
- During active food orders (for delivery tracking)
- During active rideshare trips
- When browsing nearby restaurants (with permission)

**How to Control Location:**
- iOS: Settings > Privacy > Location Services > Dollor
- Options: Never, While Using, Always

**Impact of Disabling:**
- Core features may not function without location access
- You may still use pickup orders without location

### 12.5 Biometric Data

We do not collect biometric data such as fingerprints or facial recognition data.

### 12.6 Voice Data

If you use voice features:
- Voice data is processed by Apple's speech recognition
- We do not store voice recordings
- Transcribed text may be used for search queries

### 12.7 Cookies and Tracking

Our mobile apps do not use cookies. We use:
- Device identifiers for analytics
- Advertising identifiers (with consent)

To reset advertising identifier:
- iOS: Settings > Privacy > Tracking

---

## Summary of Data Practices

| What We Collect | Why We Collect It | Who We Share With |
|-----------------|-------------------|-------------------|
| Name, email, phone | Account creation, communication | Drivers, restaurants (first name only) |
| Location | Delivery, navigation, nearby search | Drivers (during active orders) |
| Payment info | Process transactions | Stripe (payment processor) |
| Order history | Service improvement, support | Restaurants (relevant orders) |
| Device info | Security, troubleshooting | Analytics providers |

---

## Your Choices at a Glance

| Setting | How to Control |
|---------|----------------|
| Marketing emails | Unsubscribe link or app settings |
| Push notifications | Device settings or app settings |
| Location access | Device settings |
| Account deletion | App settings or email privacy@dollor.ai |
| Data download | Email privacy@dollor.ai |

---

**By using Dollor AI Service, you acknowledge that you have read and understood this Privacy Policy.**

---

*Copyright 2024 Dollor AI Service. All rights reserved.*
