...
# Optional enterprise imports (Snowflake for Coupa/NetSuite integration)
try:
    ...
    print("[INFO] Snowflake module not installed - enterprise analytics disabled")
...
def get_coupa_transactions(current_user: dict = Depends(get_current_user_with_role), days_back: int = Query(800, description="Number of days to look back for metrics."), limit: int = 100, offset: int = 0):
    if not SNOWFLAKE_ENABLED:
        return {"data": [], "message": "Snowflake not configured"}
    user_email = current_user.get("email")
    if not user_email:
        raise HTTPException(status_code=401, detail="User email not found in token")
...
def get_transactions_counts(current_user: dict = Depends(get_current_user_with_role), days_back: int = Query(800, description="Number of days to look back for metrics.")):
    if not SNOWFLAKE_ENABLED:
        return {"coupa": {}, "netsuite": {}, "message": "Snowflake not configured"}
    user_email = current_user.get("email")
    if not user_email:
        raise HTTPException(status_code=401, detail="User email not found in token")
...
# Include AI Employee Dashboard routes
from ai_dashboard_routes import router as ai_dashboard_router
