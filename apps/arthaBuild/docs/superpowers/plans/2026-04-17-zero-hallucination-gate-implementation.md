...
OUT_FILE = Path(__file__).parent.parent / "src" / "backend" / "validators" / "whitelist.py"
...
```python
def render(sets: dict[str, set[str]]) -> None:
...
def test_whitelist_not_drifted(tmp_path: Path) -> None:
    """Regenerate the whitelist into a tmp file and diff against the committed file."""
    # Patch OUT_FILE to a temp path and re-run
    spec = importlib.util.spec_from_file_location("build_whitelist", BUILD_SCRIPT)
    mod = importlib.util.module_from_spec(spec)
    mod.OUT_FILE = tmp_path / "whitelist.py"
    spec.loader.exec_module(mod)
    exit_code = mod.main()
    assert exit_code == 0, "build_whitelist floor check failed"

    regenerated = (tmp_path / "whitelist.py").read_text()
...
Note: `SuiteScriptLinter.__init__` imports checkers lazily to avoid a circular import and so tests can inject fakes. For Chunk 2, the `lint()` method with no checkers (from Wave 3) still needs to pass tests — pass an empty `checkers=[]` to the tests in Task 2.3 Step 1 if needed:

Revise the failing tests to use `SuiteScriptLinter(checkers=[])` explicitly — otherwise Task 2.3 Step 1 will fail because `RecordTypeChecker` et al. don't exist yet. Update the test file now.

- [ ] **Step 4: Revise tests to inject `checkers=[]`**
...
### Task 4.1: `reprompt_with_violations()` + `build_refusal_message()`
...
def test_reprompt_payload_includes_relevant_slice_only():
    ...
    # Injects record_type whitelist slice but NOT module/script_type/search slices
    assert "Valid record types" in payload or "record_type" in payload
```

- [ ] **Step 2: Implement**

```python
"""Re-prompt template + refusal message rendering."""
from __future__ import annotations
...
### Task 4.3: Wire into `rawapi.py` with constants

**Files:**
- Modify: `apps/arthaBuild/src/backend/rawapi.py` (around lines 542-578)

- [ ] **Step 1: Read the current `generate_suitescript` block**

```bash
sed -n '540,600p' apps/arthaBuild/src/backend/rawapi.py
```

Note the exact structure of the existing intent block — response is currently built from `run_llm_pipeline()` or equivalent; persist step is at the end.

- [ ] **Step 2: Add module constants near the top of `rawapi.py`**

After existing imports (adapt location to actual file):
...
- [ ] **Step 3: Replace the intent block (current ~542-578)**

```python
if intent == "generate_suitescript":
    # existing quota + prompt-build steps unchanged
    pipeline_t0 = time.monotonic()
    response_text = run_llm_pipeline(user_input)  # existing call
    ...
    if code is not None:
        ...
        validator_metrics["validator_elapsed_ms"] = result.elapsed_ms
        validator_metrics["violations_initial"] = len(result.violations)
        validator_metrics["categories_hit"] = sorted({v.category for v in result.violations})
        ...
        while not result.valid and attempts < REPROMPT_MAX_ATTEMPTS:
            ...
            response_text = await run_llm_pipeline(reprompt)
            code, _ = extract_first_code_block(response_text)
            if code is None:
                break
            result = linter.lint(code)
            validator_metrics[f"violations_reprompt_{attempts}"] = len(result.violations)

        if result.valid:
            validator_metrics["outcome"] = "clean" if attempts == 0 else "recovered"
        else:
            validator_metrics["outcome"] = "hard_blocked"
...
Note: the exact `run_llm_pipeline` call may be synchronous — adapt the `await` per existing code.
...
### Task 4.4: Integration test with fake generator

**Files:**
- Create: `apps/arthaBuild/tests/validators/test_integration.py`

- [ ] **Step 1: Write a test that exercises the loop with a fake LLM callable**

```python
"""End-to-end: fake LLM pipeline returns known bad/good responses; verify outcome."""
from src.backend.validators import SuiteScriptLinter, extract_first_code_block


def _fake_pipeline_that_recovers(attempt_counter: dict):
    """First call returns invalid; second returns valid."""
    def call(prompt: str) -> str:
        attempt_counter["n"] += 1
        if attempt_counter["n"] == 1:
            return "```js\nrecord.Type.RECEIVING\n```"
        return "```js\nrecord.Type.SALES_ORDER\n```"
    return call


def test_recovery_after_one_reprompt():
    counter = {"n": 0}
    pipeline = _fake_pipeline_that_recovers(counter)
    response = pipeline("initial")
    code, _ = extract_first_code_block(response)
    linter = SuiteScriptLinter()
    result = linter.lint(code)
    assert result.valid is False

    # re-prompt once
    response = pipeline("reprompt 1")
    code, _ = extract_first_code_block(response)
    result = linter.lint(code)
    assert result.valid is True


def test_hard_block_after_max_attempts():
    """Fake pipeline that always returns hallucinated code."""
    def always_bad(prompt: str) -> str:
        return "```js\nrecord.Type.RECEIVING\n```"

    linter = SuiteScriptLinter()
    for _ in range(3):  # initial + 2 re-prompts
        code, _ = extract_first_code_block(always_bad(""))
        r = linter.lint(code)
        assert r.valid is False  # every attempt fails
```
...
git commit -m "test(arthaBuild): integration test for recovery and hard-block paths"
```
...
Each case must target an identifier that is **NOT** in `RECORD_TYPES`. Spot-check each adversarial_target against the whitelist before committing.
...
- [ ] **Step 2: Write 10 plausible-but-nonexistent cases**
- [ ] **Step 3: Write 10 typo-of-real cases** (e.g. `SALE_ORDER`, `CUTOMER`, `INVOCE`)
- [ ] **Step 4: Write 10 cross-module confusion cases** (e.g. treating a search.Type as a record.Type)
...
- [ ] **Step 1: Same four-strategy distribution as Task 5.1**, but targeting `N/*` paths that are NOT in `MODULES`.
...
Mix `@NScriptType` targets (20) and `search.Type.*` targets (20). Verify each is absent from `SCRIPT_TYPES | SEARCH_TYPES`.

### Task 5.4: Author 40 `search` stress cases

**Files:**
- Create: `apps/arthaBuild/tests/eval/stress/search.jsonl`

Target `search.*` methods that are NOT in `SEARCH_APIS`: `search.columns`, `search.query`, `search.fetch`, `search.lookup`, etc.
...
- [ ] **Step 1: Ensure EC2 backend has validator code deployed**

SSH to EC2, pull branch, `docker compose up -d backend`. Verify via `/api/health/detail` that validator is importable.
...
Per project law: these three files MUST be updated as the final step of every phase.
