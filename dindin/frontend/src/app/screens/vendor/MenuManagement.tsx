...
const VendorMenuManagement: React.FC = () => {
  ...
  const fetchMenuItems = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`http://localhost:3000/api/vendors/${vendorId}/menu`);
  ...
  const handleDelete = async (itemId: number) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          await axios.delete(`http://localhost:3000/api/vendors/${vendorId}/menu/${itemId}`);
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingItem) {
        ...
        await axios.put(`http://localhost:3000/api/vendors/${vendorId}/menu/${editingItem.id}`, values);
        message.success('Menu item updated successfully');
      } else {
        // Create new item
        await axios.post(`http://localhost:3000/api/vendors/${vendorId}/menu`, values);
  ...
  const handleScrapeWebsite = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:3000/api/onboarding/scrape-menu', {
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      ...
      for (const item of selectedItems) {
        await axios.post(`http://localhost:3000/api/vendors/${vendorId}/menu`, {
          item_name: item.name,
  ...
  const fetchVerificationStatus = async () => {
    try {
      const response = await axios.get(`http://localhost:3000/api/menu-verification/status/${vendorId}`);
  ...
  const fetchBundleSuggestions = async () => {
    try {
      const response = await axios.get(`http://localhost:3000/api/menu-verification/bundle-suggestions/${vendorId}`);
  ...
  const fetchAiMessages = async () => {
    try {
      const response = await axios.get(`http://localhost:3000/api/menu-verification/messages/${vendorId}`);
  ...
  const handleUpdatePrice = async (itemId: number, newPrice: number) => {
    try {
      await axios.post('http://localhost:3000/api/menu-verification/update-price', {
        item_id: itemId,
        new_price: newPrice
      });
  ...
  const handleApproveAllPrices = async () => {
    try {
      const response = await axios.post(`http://localhost:3000/api/menu-verification/approve-all/${vendorId}`);
  ...
  const handleAcceptBundle = async (bundleId: string, customPrice?: number) => {
    try {
      ...
      await axios.post(`http://localhost:3000/api/menu-verification/accept-bundle?vendor_id=${vendorId}`, {
        bundle_id: bundleId,
        accepted: true,
  ...
  const handleActivateMenu = async () => {
    try {
      const response = await axios.post(`http://localhost:3000/api/menu-verification/activate/${vendorId}`);
  ...
  const handlePauseMenu = async () => {
    try {
      await axios.post(`http://localhost:3000/api/menu-verification/pause/${vendorId}`);
