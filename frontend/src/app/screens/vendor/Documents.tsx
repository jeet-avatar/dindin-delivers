...
import { Card, Upload, Button, Table, Tag, Space, Modal, message, Descriptions, Progress } from 'antd';
...
const VendorDocuments: React.FC = () => {
  ...
  const fetchDocuments = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`https://api.dollor.ai/api/vendors/${vendorId}/documents`);
  ...
  const handleUpload = async (file: File, documentType: string) => {
    ...
    try {
      await axios.post(`https://api.dollor.ai/api/vendors/${vendorId}/documents`, formData, {
  ...
  const handleDelete = async (documentId: number) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          await axios.delete(`https://api.dollor.ai/api/vendors/${vendorId}/documents/${documentId}`);
