...
def setup_production_admin(db: Session = Depends(get_db)):
    ...
    admin_email = os.getenv("DEMO_ADMIN_EMAIL", "support@dollor.ai")
    admin_password = os.getenv("DEMO_ADMIN_PASSWORD")

    if not admin_password:
        raise HTTPException(
            status_code=500,
            detail="DEMO_ADMIN_PASSWORD environment variable not set"
        )
...
def vendor_demo_login(request: VendorDemoLoginRequest, db: Session = Depends(get_db)):
    ...
    demo_email = os.getenv("DEMO_VENDOR_EMAIL", "demobusiness@dollor.ai")
    demo_password = os.getenv("DEMO_VENDOR_PASSWORD")

    if not demo_password:
        raise HTTPException(
            status_code=500,
            detail="DEMO_VENDOR_PASSWORD environment variable not set"
        )
...
def setup_demo_accounts(db: Session = Depends(get_db)):
    ...
    # --- Demo Customer ---
    try:
        customer_email = os.getenv("DEMO_CUSTOMER_EMAIL", "demo.customer@dollor.ai")
        ...
        if not existing:
            if not customer_password:
                results["errors"].append("customer: DEMO_CUSTOMER_PASSWORD env var not set")
            else:
    ...
    # --- Demo Driver ---
    try:
        driver_email = os.getenv("DEMO_DRIVER_EMAIL", "demo.driver@dollor.ai")
        ...
        if not existing:
            if not driver_password:
                results["errors"].append("driver: DEMO_DRIVER_PASSWORD env var not set")
            else:
                demo_driver = Driver(
    ...
    try:
        vendor_email = os.getenv("DEMO_RESTAURANT_EMAIL", "demo.restaurant@dollor.ai")
        vendor_password = os.getenv("DEMO_RESTAURANT_PASSWORD")
        existing = db.query(Vendor).filter(Vendor.contact_email == vendor_email).first()
        if not existing:
            if not vendor_password:
                results["errors"].append("restaurant: DEMO_RESTAURANT_PASSWORD env var not set")
            else:
                demo_vendor = Vendor(
                    restaurant_name="Demo Restaurant",
                    company_name="Demo Restaurant LLC",
                ...
                vendor_user = User(
                    email=vendor_email,
                    password_hash=get_password_hash(vendor_password),
                    full_name="Demo Owner",
                    role=UserRole.VENDOR,
                    vendor_id=demo_vendor.id,
                    created_at=datetime.utcnow()
    ...
    try:
        ...
        admin_password = os.getenv("DEMO_ADMIN_PASSWORD")
        if not admin_password:
            results["errors"].append("admin: DEMO_ADMIN_PASSWORD env var not set")
        else:
            existing = db.query(User).filter(User.email == admin_email).first()
            if not existing:
                admin_user = User(
    ...
    return {
        ...
        "results": results,
        "credentials": {
            "customer": {"email": os.getenv("DEMO_CUSTOMER_EMAIL", "demo.customer@dollor.ai"), "password": "[set in env]"},
            "driver": {"email": os.getenv("DEMO_DRIVER_EMAIL", "demo.driver@dollor.ai"), "password": "[set in env]"},
            "restaurant": {"email": os.getenv("DEMO_RESTAURANT_EMAIL", "demo.restaurant@dollor.ai"), "password": "[set in env]"},
            "admin": {"email": os.getenv("DEMO_ADMIN_EMAIL", "support@dollor.ai"), "password": "[set in env]"}
        },
        "note": "Passwords are configured via environment variables. Check .env.example for required variables."
    }
