...
- [x] **AUTH-03**: All vendor endpoints have per-endpoint Depends(require_vendor) with ownership checks
- [x] **AUTH-04**: All admin endpoints have per-endpoint Depends(require_admin) role checks
- [x] **AUTH-05**: All remaining middleware-only endpoints converted to per-endpoint Depends()
- [x] **AUTH-06**: Zero endpoints rely solely on global middleware for auth (all have explicit Depends)
