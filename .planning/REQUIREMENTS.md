...
| DrawingPdfGenerator | 60-04 | Complete (NEW repo `github.com/jeet-avatar/zietra-cad-pdf-gen` (private) + NEW Lambda `zietra-cad-pdf-gen` ARN `arn:aws:lambda:us-east-1:134607809447:function:zietra-cad-pdf-gen` (x86_64, 2048 MB, 60s, in vpc-012ab4500dcd4ee41 with separate IAM role `zietra-cad-pdf-gen-lambda-role`) running Puppeteer + @sparticuz/chromium@^131. Final CodeSha256 `64f64b295326107a810533606a3add0f49f0f1ba2cecd7665e98577a36f141f5`. NEW SQS queue `zietra-cad-pdf-gen-queue` (90s visibility, 4d retention) + DLQ `zietra-cad-pdf-gen-dlq` (14d, maxReceive=3) + Lambda event source mapping (batchSize=1). NEW S3 gateway VPC endpoint `vpce-0e8a0a9ac582df244` on the satellite VPC (private route table) — Rule-3 auto-fix since the VPC had no S3 endpoint and PUTs via NAT instance timed out under load. NEW satellite routes: POST `/api/parts/:id/drawings/generate` (admin/manager, INSERTs part_drawing_jobs status=queued + sqs.send + audits cad_pdf_generate stage=enqueued, returns 202 with job_id + poll_url) + GET `/api/cad-pdf-jobs/:jobId` (any auth, returns job row + 15-min presigned URL when status=ready). Satellite-api Lambda CodeSha256 `90313b4b63788e8a405fd16a771e6e8e89bf5b40028744dd3d884e76d04fdcce`. **PITFALL 5 DISPATCH AT PDF RENDER TIME**: handler.mjs queries part_cad_files FIRST; if uploaded exists, PDF embeds `Uploaded CAD: <file> · rev <N> · <format>` + presigned download link + markup overlay; only falls back to part_definitions.drawing_svg when no upload. SAME rule that Plan 60-02 enforced on the on-screen viewer. Cross-cutting smoke script `scripts/smoke-phase-60.sh` (362 lines) walks a robotic-arm part end-to-end through every layer, provisions+tears-down a brand-new test tenant, downloads the rendered PDFs, extracts text via pypdf, asserts dispatch correctness; first-run result **34 pass / 0 fail / 0 errors**. Robotic-arm walkthrough proven: extracted text from uploaded-branch PDF contains `Uploaded CAD: robot-arm.step · rev 1 · step` + markup overlay `Joint pin location` + does NOT contain `Procedural template`. Procedural-only branch PDF contains `Procedural template · rev 3` + does NOT contain `Uploaded CAD`. Live HTTPS auth gates verified: POST /drawings/generate (no auth) → 401, GET /cad-pdf-jobs/:id (no auth) → 401, no tenant slug → 400. Substitute pattern for the Cognito-bound smoke matches 60-01/02/03 — operator full visual walk remains gated. Audit log captures `payload->>'source'` = uploaded/procedural with stage=rendered. The S3 gateway endpoint side-benefit also speeds up the existing satellite-api S3 presign + HEAD calls.) |

---
*Requirements defined: 2026-02-26*
*Last updated: 2026-05-16T11:20Z -- **Phase 60 (Real CAD support: STEP/STL upload + 3D viewer + drawing markup + PDF generation) COMPLETE — 4/4 plans + 10/10 requirement IDs closed.** Migrations 023+024 in turion-satellite. NEW S3 bucket `zietra-cad-files-134607809447`. NEW Lambda `zietra-cad-pdf-gen` (separate repo + IAM). NEW SQS queue + DLQ + ESM + S3 gateway VPC endpoint. Robotic-arm walkthrough smoke 34/34 PASS. Pitfall 5 dispatch (uploaded > procedural) operational at both on-screen viewer AND printed-drawing PDF render target. Ready for a robotics customer to onboard today. Phase 60 CHECKPOINT.md hands off 8 deferred items to Phase 61 + 3 next-step prompts (Phase 61 CAD enrichments / Phase 56 Stripe billing / Phase 62 M9 GA readiness). Earlier 2026-05-15T20:58Z note: Phase 55 (M3 multi-tenancy + RLS) COMPLETE — all 5 plans + 7 requirement IDs closed.*
