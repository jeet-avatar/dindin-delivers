...
Plans:
- [ ] 38-01-PLAN.md — Backend requireAuth middleware + secrets.ts + lambda.ts loadSecrets() + apply requireAuth per-route across 12 routers (commit only, no deploy)
- [ ] 38-02-PLAN.md — Frontend helpers (erp-auth.js + erp-api.js + erp-login.html + erp-auth-callback.html) + generate-turion-config.sh emits Supabase URL + anon key (commit only)
- [ ] 38-03-PLAN.md — Migrate 61 fetch sites (5 shared-JS + 56 HTML) to window.erpApi.* + inject requireSession() guard on 79 ERP HTML pages (commit only)
- [ ] 38-04-PLAN.md — Audit-script extension + Lambda env+IAM update + push + deploy backend+frontend + CF invalidation + curl smoke + Supabase URL allowlist checkpoint + headless JWT round-trip + STATE/ROADMAP update
