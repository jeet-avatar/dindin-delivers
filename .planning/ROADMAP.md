...
**Plans:** 9/9 plans complete (2026-05-12) — the prod-infra blocker (the shared Supabase DB password) was resolved (password rotated + updated in the `turion-satellite/production/database-url` secret AND `turion-demo-api`'s `DATABASE_URL` env var; both Lambdas bounced) → the DB-direct E2E walk per module passed. Headless-substitute checkpoint approved. Remaining user nice-to-haves (NOT blockers): (1) rotate the exposed Resend key `re_JRdox6wH_…` + set `RESEND_API_KEY` on `turion-demo-api`; (2) create the Phase-34 `turion-satellite/production/anthropic-key` secret + `ANTHROPIC_API_KEY_ARN` on `turion-satellite-api`; (3) a future backend plan to add `turion.*` tables for the FP&A/COA/company/tax/users config so the last ~6 config-only `*-data.js` static files can be deleted.
