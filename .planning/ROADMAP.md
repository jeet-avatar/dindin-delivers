...
- [ ] 31-02-PLAN.md — satellite-3d.js: extend mount3DViewer with `opts.assemblyChildren` + `opts.onSelect` — radial-ring multi-mesh layout (one buildPartMesh per child), THREE.Raycaster picker (canvas-rect NDC), emissive hover highlight, camera fly-to tween, viewerHandle.deselect(), dispose() listener cleanup (leaf single-mesh path untouched)
