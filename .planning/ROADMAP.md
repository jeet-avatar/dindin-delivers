...
Plans:
- [ ] 36-01-PLAN.md — satellite backend: confirm /api/lifecycle-stages shape + add any missing satellite lookup endpoint (likely a no-op verify pass) + tests [W1]
- [ ] 36-02-PLAN.md — ERP backend: GET /api/{arena,netsuite,salesforce,mes}/lookups + confirm /api/data/all completeness + turion-config.js (generated, single source of truth) + replace ~12 copy-pasted const API_BASE [W1]
- [ ] 36-03-PLAN.md — satellite frontend: replace PROGRAM_STAGES with /api/lifecycle-stages + spot-check the Phase-33 PLM flow + button audit clean [W2]
- [ ] 36-04-PLAN.md — ERP de-hardcode Salesforce + NetSuite: remove static *-data.js includes, render from /api/data/all (loud retry on error), form dropdowns from /lookups, delete orphaned data files [W2]
- [ ] 36-05-PLAN.md — ERP de-hardcode Arena + MES + integration: remove static *-data.js includes, /api/data/all, /api/{arena,mes}/lookups dropdowns, delete orphaned files + minimal MES read endpoint only if mes-shop-floor.html needs it [W2]
...
- [ ] 36-07-PLAN.md — WIP resolution: move notify.ts's plaintext Resend key to AWS Secrets Manager + a Lambda env var (scrub from src/ + dist/), untrack backend/node_modules, confirm lambda-build diff benign, commit the 3-AI-agents feature + agent-sales-cash/dashboard-cio/about-this-demo.html [W3]
