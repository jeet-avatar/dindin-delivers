...
Plans:
- [ ] 37-01-PLAN.md — Backend foundation: migration 023 + qb_records/ramp_card_txns/migration_runs DDL + ~170-row seed + GET routes + FIELD_MAPS const + keyedEntity for bills/gl_accounts + mount in app.ts
- [ ] 37-02-PLAN.md — POST /migrate routes: applyMapping for all 6 QB types + Ramp applyRampMapping + atomic transactions + audit trail (migration_runs + audit_log) + idempotent skipped[]
- [ ] 37-03-PLAN.md — Frontend wizard: 8 HTML pages (landing + 6 per-type wizards + ramp.html) with 3-pane CSS-grid layout + index.html migration-tools section + 8 CF clean-URL rewrites
- [ ] 37-04-PLAN.md — Audit + deploy (push both repos, redeploy turion-demo-api Lambda, deploy-frontend.sh + F6 pre-flight, CF invalidation) + DB-direct E2E walk + STATE/ROADMAP + headless-substitute checkpoint
...
*Last updated: 2026-05-12 -- Phase 36 COMPLETE (9/9 plans): zero hardcodes + E2E audit across the whole Turion Space demo (satellite PLM + Arena/Salesforce/NetSuite/MES) — both frontends de-hardcoded (PROGRAM_STAGES/SAT_STATUSES → API; ERP *-data.js de-referenced, pages render from /api/data/all; turion-config.js single API-base source), lookup endpoints added (/api/{module}/lookups + satellite-statuses, degrade to canonical lists), ERP persistence (ns-editable.js → PATCH; primary state-change buttons per module; 3 AI agents → real DB writes), WIP resolved (agents committed, plaintext Resend key scrubbed), button audit 0/0 on both frontends. Both repos pushed, both Lambdas redeployed (turion-satellite-api 2984d8e9→1134cefc, turion-demo-api fd4605e6→c716f0d2), frontend deployed + CF invalidated (IC6IW03ZMEJE46DK93BO7XVI6B → Completed). The pre-existing Supabase-DB-password blocker was resolved out-of-band (rotated + updated in the secret + the Lambda env, both Lambdas bounced, /api/health → {db:ok}) → the DB-direct E2E walk per module passed (satellite PLM spot-check intact; SF/NS/Arena/MES create→read→update round-trips via the real backend routes, all cleaned to baseline). Headless-substitute checkpoint approved. Remaining user nice-to-haves: rotate the exposed Resend key + set RESEND_API_KEY on turion-demo-api; create the Phase-34 turion-satellite/production/anthropic-key secret + ANTHROPIC_API_KEY_ARN on turion-satellite-api; future turion.* config tables. (prev) Phase 35 COMPLETE (7/7 plans): editable CAD drawings (in-browser SVG editor + server-side Phase-27-generator "revert") + part management (create/edit/retire parts, delete BOM lines) + migration 022. Lambda redeployed, frontend redeployed, headless-substitute checkpoint approved.*
